...

{
  ...
  try {
    const { generateThumbnail } = require('./lib/generate-thumbnail.cjs')
      { ...worksheetData, skill_url: skillUrl, id: data.id },
      subSkill,
      node
    )
    const fileName = `thumbnails/${data.id}.png`
    ...
    if (!uploadError) {
      ...
      await supabase.from('worksheets').update({ thumbnail_url: thumbnailUrl }).eq('id', data.id)
    }
  } catch (thumbErr) {
...
async function run() {
  try {
    // 1. Find which sub-skills already have worksheets
